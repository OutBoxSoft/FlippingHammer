...
//Rare exceptions to indicate a bug in the _flippinghammer code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_flippinghammer = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_flippinghammer]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.flippinghammer.module"
